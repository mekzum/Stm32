...
int main ()
{
  ...
  RandomInit();
	...
	while(1)
	{
		...
		LcdWriteInt(120,(int)Target[i]);
		LcdWriteInt(170,RandomCreate());
		i++;
...
void RandomInit (){	
	RCC->AHB2ENR |=0x40;  //Random Clock Enable
	RNG->CR = 0x4; //RNG Enable

}
int RandomCreate()
{

	while(!(RNG->SR&1));  //Wait until RNG ready!
	return RNG->DR;     //return value     
